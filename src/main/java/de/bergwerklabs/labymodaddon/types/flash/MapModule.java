 ...
 * @author Nico_ND1
 */
public class MapModule extends SimpleTextModule {
    ...
    public boolean isShown() {
        return LabyBergwerk.getInstance().getFlash().isEnabled() && getBooleanElement().getCurrentValue() && (isAutor() || isName());
    }

    @Override
    public String[] getValues() {
        if (LabyBergwerk.getInstance().getFlash().getMap() != null) {
            if (isAutor() && isName()) {
                return new String[]{LabyBergwerk.getInstance().getFlash().getMap().getAuthor(), LabyBergwerk.getInstance().getFlash().getMap().getName()};
            } else if (isAutor()) {
                return new String[]{LabyBergwerk.getInstance().getFlash().getMap().getAuthor()};
            } else if (isName()) {
                return new String[]{LabyBergwerk.getInstance().getFlash().getMap().getName()};
            }
        }
        return new String[]{};
    }
